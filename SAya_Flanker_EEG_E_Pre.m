...
eeglab;
m_path = '/Volumes/Data/ZShared/SNEACY/2_SNEACY_YA/SNEACY_YA_EEG/Flanker_EEG/Flanker_EEG_Exercise/Pre/';  % This is the path to your main processing folder
...
% Parsing data into flanker, Go, and NoGo files 
    % Select data within start/end triggers
...
filelist = dir('SAya_*_E_Flanker_avg.erp');
...
ERP = pop_savemyerp(ERP, 'erpname', 'SAya_pr_Flanker_E_', 'filename', 'SAya_pr_Flanker_E_.erp', 'filepath', [m_path 'ERPsets/'], 'Warning', 'off');   
pop_export2text( ERP, [m_path 'Export_Excel/GrandAVG/','SAya_pr_Flanker_E_','_erp_export.txt'], [13 14], 'electrodes', 'on', 'precision',4, 'time', 'on', 'timeunit',0.001 );
% % 
% disp(',');
% disp('--------------------------------');
% disp(['Grand-averaged ' num2str(length(filenames)) ' Participants Flanker Pre Exercise']); 
% disp('--------------------------------');
% disp(',');
% clear('filenames');
% % 
